...
def highway(input_, size, num_layers=1, bias=-2.0, f=tf.nn.relu, scope=None):
    ...
    with tf.variable_scope('Highway/' + scope):
...
def tdnn(input_, kernels, kernel_features, scope):
    '''
    ...
    layers = []
    with tf.variable_scope('TDNN/' + scope):
...
def cnn_sen_enc(word_vocab_size,
    ...
    question_ = tf.placeholder(tf.int32, shape=[batch_size, max_sen_length], name="question")
    ...
    with tf.variable_scope('Embedding'):
        ...
        question_embeded = tf.nn.embedding_lookup(word_embedding, question_)
        with tf.variable_scope('Word-level_ompare-Aggregate'):
            input_embedded, question_embeded = word_compare_aggregate(input_embedded, question_embeded,\
                                                            batch_size, max_doc_length, max_sen_length)

        input_embedded = tf.reshape(input_embedded, [-1, max_sen_length, word_embed_size])
        question_embeded = tf.reshape(question_embeded, [-1, max_sen_length, word_embed_size])


    ''' Second, apply convolutions '''
    # [batch_size x max_doc_length, cnn_size]  # where cnn_size=sum(kernel_features)
    input_cnn = tdnn(input_embedded, kernels, kernel_features, scope='input')
    question_cnn = tdnn(question_embeded, kernels, kernel_features, scope='question')

    ''' Maybe apply Highway '''
    if num_highway_layers > 0:
        input_cnn = highway(input_cnn, input_cnn.get_shape()[-1], num_layers=num_highway_layers, scope='input')
        question_cnn = highway(question_cnn, question_cnn.get_shape()[-1], num_layers=num_highway_layers, scope='question')
    ...
    return adict(
        ...
        question_embeded=question_embeded,
        input_cnn=input_cnn,
        question_cnn=question_cnn,
    )

def word_compare_aggregate(input_embedded, question_embeded, batch_size, max_doc_length, max_sen_length):
    reshape_question_embeded = tf.tile(tf.expand_dims(question_embeded,1),[1,max_doc_length,1,1])
    att_mat = tf.matmul(reshape_question_embeded, tf.transpose(input_embedded, [0,1,3,2]))
    row_max = tf.reduce_max(att_mat, axis=2)
    row_max = tf.expand_dims(tf.nn.softmax(row_max),-1, name='answer_attention')
    column_max = tf.expand_dims(tf.nn.softmax(tf.reduce_max(att_mat, axis=3)),-1, name='question_attention')
    att_input_embedded = tf.reshape(tf.multiply(input_embedded,row_max),[batch_size, max_doc_length, max_sen_length, -1])
    att_question_embedded = tf.reshape(tf.multiply(reshape_question_embeded,column_max),[batch_size, max_doc_length, max_sen_length, -1])

    att_input_embedded = tf.multiply(att_input_embedded, reshape_question_embeded)
    att_question_embedded = tf.multiply(att_question_embedded, input_embedded)
    return att_input_embedded, att_question_embedded


def sent_compare_aggregate(input_cnn, question_cnn, batch_size, max_doc_length):
    reshape_input_cnn = tf.transpose(input_cnn, [0,2,1])
    att_mat = tf.matmul(question_cnn, reshape_input_cnn)
    ...
    att_answer_outputs = tf.reshape(tf.multiply(input_cnn,row_max),[batch_size, max_doc_length, -1])
    att_question_outputs = tf.reshape(tf.multiply(question_cnn,column_max),[batch_size, max_doc_length, -1])

    att_answer_outputs = tf.multiply(att_answer_outputs, question_cnn)
    att_question_outputs = tf.multiply(att_question_outputs, input_cnn)
    return att_answer_outputs, att_question_outputs
    

def bilstm_doc_enc(input_cnn, question_cnn,
                       batch_size=20,
                       num_rnn_layers=2,
                       rnn_size=150,
                       max_doc_length=35,
    ...
    with tf.variable_scope('BILSTMenc'):
        ...
        question_cnn = tf.reshape(question_cnn, [batch_size, max_doc_length, -1])
        question_cnn2 = [tf.squeeze(x, [1]) for x in tf.split(question_cnn, max_doc_length, 1)]
        print(input_cnn2.get_shape(), question_cnn2.get_shape())

        with tf.variable_scope('Sentence-level_Compare-Aggregate'):
            input_cnn2, question_cnn2 = sent_compare_aggregate(input_cnn2, question_cnn2, batch_size, max_doc_length)
...
def lstm_doc_enc(input_cnn, question_cnn,
                   batch_size=20,
                   num_rnn_layers=2,
                   rnn_size=150,
                   max_doc_length=35,
    ...
    with tf.variable_scope('LSTMenc'):
        ...
        print(input_cnn.get_shape(),question_cnn.get_shape())

        with tf.variable_scope('Sentence-level_Compare-Aggregate'):
            input_cnn, question_cnn = sent_compare_aggregate(input_cnn, question_cnn, batch_size, max_doc_length)
        ...
        print(np.asarray(input_cnn2).shape,np.asarray(question_cnn2).shape)
        ...
        with tf.variable_scope('/question'):
            question_outputs, _ = tf.contrib.rnn.static_rnn(cell, question_cnn2,
                                         initial_state=initial_rnn_state, dtype=tf.float32)
            #question_outputs = tf.reduce_mean(tf.transpose(question_outputs, [1,0,2]),1)

        print(np.asarray(outputs).shape,np.asarray(question_outputs).shape)
...
def label_prediction_att(outputs_enc, outputs_dec, question_outputs):

    # scoring labels with att
    logits = []
    with tf.variable_scope('Prediction') as scope:
        for idx, output in enumerate(zip(outputs_enc, outputs_dec, question_outputs)):
            if idx > 0:
                scope.reuse_variables()
            output_enc, output_dec, question_output = output
            logits.append(linear(tf.concat([output_enc, output_dec, question_output], 1), 2))
    return adict(
        logits = logits
    )
