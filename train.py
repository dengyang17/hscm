...
flags.DEFINE_string ('embedding_path',  '../../embed/glove.6B.100d.txt',                           'pretrained emebdding path')
...
def build_model(word_vocab, max_doc_length, train):
    ...
    if train:
        ...
        if FLAGS.model_choice == 'bilstm':
            ...
            my_model.update(model.bilstm_doc_enc(my_model.input_cnn, my_model.question_cnn,
                                           batch_size=FLAGS.batch_size,
            ...
            my_model.update(model.lstm_doc_enc(my_model.input_cnn, my_model.question_cnn,
                                           batch_size=FLAGS.batch_size,
                                           num_rnn_layers=FLAGS.rnn_layers,
                                           rnn_size=FLAGS.rnn_size,
                                           max_doc_length=max_doc_length,
            ...
            my_model.update(model.label_prediction_att(my_model.enc_outputs, my_model.dec_outputs, my_model.question_outputs))
            my_model.update(model.loss_extraction(my_model.logits, FLAGS.batch_size, max_doc_length))
...
def main(_):
    ...
    word_vocab, word_tensors, max_doc_length, label_tensors, question_tensors, _ = \
    ...
    train_reader = DataReader(word_tensors['train'], label_tensors['train'],
                              question_tensors['train'], FLAGS.batch_size)

    valid_reader = DataReader(word_tensors['test'], label_tensors['test'],
                              question_tensors['test'], FLAGS.batch_size)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        for epoch in range(FLAGS.max_epochs):
            ...
            avg_train_loss = 0.0
            count = 0
            for x, y, q in train_reader.iter():
                count += 1
            ...
            for x, y, q in valid_reader.iter():
                count += 1
                start_time = time.time()
