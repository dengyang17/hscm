...
flags.DEFINE_integer('rnn_size',        150,                            'size of LSTM internal state')
flags.DEFINE_integer('highway_layers',  2,                              'number of highway layers')
flags.DEFINE_integer('word_embed_size', 100,                             'dimensionality of word embeddings')
flags.DEFINE_string ('kernels',         '[1,2,3,4,5,6,7]',              'CNN kernel widths')
flags.DEFINE_string ('kernel_features', '[50,50,100,100,100,100,100]', 'number of features in the CNN kernel')
...
flags.DEFINE_integer('max_sen_length',    30,   'maximum sentence length')
...
def build_model(word_vocab, max_doc_length):
    if FLAGS.model_choice == 'bilstm':
        ...
        valid_model.update(model.bilstm_doc_enc(valid_model.input_cnn, valid_model.question_cnn,
                                           batch_size=FLAGS.batch_size,
        ...
        valid_model.update(model.loss_extraction(valid_model.logits, FLAGS.batch_size, max_doc_length)) 
        ...
        valid_model.update(model.lstm_doc_enc(valid_model.input_cnn, valid_model.question_cnn,
                                           batch_size=FLAGS.batch_size,
                                           num_rnn_layers=FLAGS.rnn_layers,
                                           rnn_size=FLAGS.rnn_size,
                                           max_doc_length=max_doc_length,
        ...
        valid_model.update(model.label_prediction_att(valid_model.enc_outputs, valid_model.dec_outputs, valid_model.question_outputs))
        valid_model.update(model.loss_extraction(valid_model.logits, FLAGS.batch_size, max_doc_length))
...
def main(_):
    ...
    word_vocab, word_tensors, max_doc_length, label_tensors, question_tensors, sentences = \
        load_data(FLAGS.data_dir, FLAGS.max_doc_length, FLAGS.max_sen_length, data_set=['test'])

    test_reader = DataReader(word_tensors['test'], label_tensors['test'], question_tensors['test'],
                              FLAGS.batch_size)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        with tf.variable_scope("Model"):
            m = build_model(word_vocab, max_doc_length)
        ...
        print(len(result_scores),len(result_scores[0]))
        print(len(sentences['test']),len(sentences['test'][0]))
        with open('%s/result.txt' % (FLAGS.train_dir),'w',encoding='utf-8') as outfile:
            for doc, scores in zip(sentences['test'], result_scores):
                tmp = zip(doc, scores)
